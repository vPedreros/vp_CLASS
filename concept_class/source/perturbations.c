...
#include <stdio.h> /* Added for printing stuff into txt file*/
...
int perturb_prepare_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      class_store_columntitle(ppt->scalar_titles, "shear_fld", pba->has_fld);
      /**
...
int perturb_vector_init(
  ...
  if (_scalars_) {
    ...
    if (pba->use_ppf == _FALSE_) {
      ...
      /* vPedre: Below added for shear*/
      class_define_index(ppv->index_pt_shear_fld,pba->has_fld,index_pt,1); /* fluid velocity */
    }
    else {
    ...
    if (_scalars_) {
      ...
      if (pba->has_fld == _TRUE_) {
        ...
        if (pba->use_ppf == _FALSE_) {
          ...
          /* vPedre: Below added for shear*/
          ppv->y[ppv->index_pt_shear_fld] =
            ppw->pv->y[ppw->pv->index_pt_shear_fld];
        }
        else {
  ...
  if (_scalars_) {
    ...
    if (pba->has_fld == _TRUE_) {
      ...
      if (pba->use_ppf == _FALSE_){
        ...
        cs2 = 0.1; /*vPedre: Domenico said*/
        cvis2 = 0.2; /* vPedre: added for shear*/
        ...
        if (ppt->gauge == synchronous) {
          dy[pv->index_pt_theta_fld] = /* fluid velocity */
            ...
            +y[pv->index_pt_shear_fld]; /* vPedre: Below added for shear*/
        }
        else {
          dy[pv->index_pt_theta_fld] = /* fluid velocity */
            -(1.-3.*cs2)*a_prime_over_a*y[pv->index_pt_theta_fld]
        ...
        /** - ----> fluid shear */
        /* vPedre: Below added for shear*/
        dy[pv->index_pt_shear_fld] = 
          -3.*a_prime_over_a*y[pv->index_pt_shear_fld]/a
          + 8./3.*cvis2*y[pv->index_pt_theta_fld]/(1+w_fld)/(1+w_fld)/a;
